...
Reads and in-project file edits run without review. Shell commands,
out-of-project edits, subagent launches, web fetches, and web searches go to
the classifier. Classifier denials return `Blocked by classifier` feedback to
the agent, and explicit `"deny"` rules are still enforced, including over a
saved approval. Classifier failures deny by default.
...
See the [auto mode configuration](/docs/config#reviewed-auto-mode) for
`permission_auto` rules, trusted environments, and the
`opencode auto-mode defaults|config|critique|reset` commands.
