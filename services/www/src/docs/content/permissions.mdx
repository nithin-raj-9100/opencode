...
Clients may attach feedback when rejecting.

## Reviewed auto mode

Reviewed auto mode classifies consequential tool calls with an isolated
side query while the agent keeps working. Enable it with the permission-mode
command or start the TUI with `--auto`.

Reads of any file or folder — including `.env`, `.git`, and home paths such as
`~/.zshrc` — are allowed by default. That covers `read`, `glob`, `grep`,
directory listings, and the `external_directory` gate those tools use for
home paths. In-project file edits are auto-approved. Shell commands,
out-of-project edits, subagent launches, web fetches, and web searches go to
the classifier even when agent rules would otherwise allow them. A configured
`deny` still wins, including over a saved approval.

The classifier receives user messages, assistant tool calls, the proposed
action, and its resources. It does not receive assistant prose, reasoning, or
tool results. It returns one of three decisions:

- **Allow** approves only the current request.
- **Deny** rejects the request with `Blocked by classifier` feedback so the
  agent can choose a safer approach. Do not retry or route around that denial.
- **Ask** shows the normal human approval prompt and keeps auto mode running.

After three consecutive denials or 20 total denials, auto mode keeps running
but sends remaining consequential actions to you instead of the classifier.
Approving one of those prompted actions resumes the classifier for the session
family. Pending requests from child sessions are reviewed through their root
session. Classifier API failures deny when the iron gate is closed and fall
back to a human prompt when it is open.

When the classifier request fails, the denial reason names the underlying
failure when one is known, for example `classifier unavailable: provider
overloaded`. The server log records the full cause under `automatic permission
review unavailable`. Set `OPENCODE_AUTO_IRON_GATE=open` to fall back to a
human prompt instead of denying while the classifier is unreachable, and set
`permission_auto.model` to a model the background service can reach.

The classifier runs on the configured `model` when set and otherwise on the
session model, with the session model as a fallback when a configured model is
unavailable. Every review is sent with a rule-derived cache key so repeated
contexts reuse provider cache.

While auto mode is active, broad allow rules such as `* / *` and `shell *` are
suspended so commands reach the classifier. A blanket `shell *` ask is the
same remaining default: the classifier decides. Narrow allow rules such as
`cat *` still skip the classifier even when that blanket ask is also present.
Narrow ask rules such as `git push *` still prompt you. Critical removals such
as `rm -rf /` are denied deterministically.

While auto mode is active, a separate prompt-injection probe inspects textual
tool results and prepends a security warning to suspicious output before the
next model step. Configure the classifier and probe with
[`permission_auto`](/config#reviewed-auto-mode).

Non-interactive clients must decide
how to handle approval requests; configured `deny` rules always remain enforced.

`opencode run --auto` classifies each request the same way. Classifier denials
are printed and the run continues. Human escalations abort the run and exit
with a non-zero status. Explicit `deny` rules remain enforced.
