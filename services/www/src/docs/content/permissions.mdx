...
home paths. Web fetches and web searches skip the classifier too. In-project
file edits are auto-approved. Shell commands, out-of-project edits, and
subagent launches go to the classifier even when agent rules would otherwise
allow them. A configured `deny` still wins.
...
Pending requests from child sessions are reviewed through their root session.
Classifier API failures deny when the iron gate is closed and fall back to a
human prompt when it is open.
...
are printed and the run continues. Human escalations abort the run. Explicit
`deny` rules remain enforced.
