...
The V2 TUI can classify requests that remain `ask` after configured and saved
permission rules run. Enable it with the permission-mode command or start the
TUI with `--auto`.
...
- **Deny** rejects the request with feedback so the agent can choose a safer
  approach.
- **Ask** pauses auto mode and shows the normal human approval prompt.

Auto mode also pauses for human review after three consecutive denials or 20
total denials. Pending requests from child sessions are reviewed through their
root session, so subagent handoffs do not bypass the policy. Classifier failures
fail closed to the normal human prompt.

While auto mode is active, broad shell allow rules are suspended so commands
reach the classifier, and read-only tools skip it without a model call.
Critical removals such as `rm -rf /` are denied deterministically.
...
`opencode run --auto` classifies each request the same way and aborts the run
after repeated denials. Explicit `deny` rules remain enforced.
