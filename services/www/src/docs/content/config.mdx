...
file or folder skip the classifier; shell, out-of-project edits, and subagents
are classified. The selected model must be available to the background service.

```jsonc
{
  "permission_auto": {
    "model": "openai/gpt-5.6-luna#high",
    "environment": "Only this repository and the acme GitHub organization are trusted.",
...
cannot widen its own policy. Inspect the rules with:
...
`transport: "websocket"` on a provider or model selects its session
