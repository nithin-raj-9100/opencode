...
Configure the classifier used when reviewed auto mode is active. Reads of any
file or folder skip the classifier; shell, out-of-project edits, subagents,
web fetches, and web searches are classified. The selected model must be
...
```jsonc
{
  "permission_auto": {
    ...
    "environment": [
      "$defaults",
      "Only this repository and the acme GitHub organization are trusted.",
    ...
    "allow": ["$defaults", "Deploying to staging is allowed: it resets nightly."],
...
`soft_deny` blocks unless explicit user intent or an `allow` exception clears
it. `hard_deny` blocks unconditionally. `block` remains as an alias for
`soft_deny`; when both are set their entries merge. `environment` accepts a
single string or an array of entries, each of which may contain `"$defaults"`.

```jsonc
{ "permission_auto": { "soft_deny": ["Never touch prod."] } }
```

Include `"$defaults"` to keep the built-in rules; without it the list replaces
the defaults entirely. `classifyAllShell` routes every shell command through
the classifier. `classifier` selects `"both"`, `"fast"`, or `"thinking"`.
`disableAutoMode` rejects every enable request, including `--auto`, so an
administrator can turn the feature off in user or managed settings. When a
configured `model` is unavailable the classifier falls back to the session
model instead of failing closed.

`permission_auto` from project settings is ignored, so a checked-in repo
cannot widen its own policy. Inspect, review, and reset the rules with:

```sh
opencode auto-mode defaults
opencode auto-mode config
opencode auto-mode critique
opencode auto-mode reset --yes
```

See [Reviewed auto mode](/permissions#reviewed-auto-mode) for the decision and
escalation behavior.
