...
```jsonc
{
  "permission_auto": {
    ...
    "allow": ["Push to the current non-protected feature branch."],
    "prompt_injection_probe": true,
  },
}
```

`block` and `allow` extend the built-in policy; they do not replace configured
permission rules. Omit `model` to use the active session model. The prompt
injection probe checks textual tool results while reviewed auto mode is active
and is enabled unless set to `false`.
